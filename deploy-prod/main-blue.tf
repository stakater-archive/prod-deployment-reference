...
module "prod-blue-group-deployer" {
  ...
  # Resource tags
  name = "${var.app_name}-${var.environment}-blue-group"
...
resource "aws_security_group_rule" "blue-group-sg-deployer-app-ssl" {
  count                    = "${signum(length(var.ssl_certificate_id))}" # if ssl_certificate_id is set, this will result in 0 and will create non-ssl resource
  type                     = "ingress"
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
